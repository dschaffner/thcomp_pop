...
%
\begin{equation}
\gamma_{s} = \frac{\Delta r_{0}}{\Delta \theta} \frac{\partial}{\partial r}\left(\frac{V_{\theta}}{r}\right)
\label{eq:shearfull}
\end{equation}
%
with $\Delta r_{0}$ is the radial correlation length and $r\Delta \theta$ the azimuthal correlation length. A correction of the order $V_{\theta}/r^{2}$ can be dropped out for radii on the order of 30cm as in this experiment. The measured ratio of $\Delta r_{0}/r\Delta \theta$ is generally close to one, but can vary with bias. Nevertheless, for experimental simplicity, the shearing rate for the data presented here is computed as $\gamma_{s} = d/dr(V_{\theta})$.

The normalization, $\tau_{ac}$, is determined by finding the width of the autocorrelation function of the density time series at zero shearing rate. FFT quantities are calculated over a time range of 3.2ms. The plasma potential is determined from voltage sweeps 2.5ms long.
